...
export function PublicMenuPage(){
  ...
  <Header meta={meta} restaurantId={restaurantId} tableId={tableId} t={t} onToggleLanguage={()=> setLanguage(l=> l==='ar' ? 'fr' : (l==='fr' ? 'en' : 'ar'))} cartCount={cartCount} hasActiveOrder={!!activeOrderId} onOpenCart={()=> setShowCart(true)} onOpenTracker={()=> setShowOrderTracker(true)} />
      ...
      <FloatingActions cartCount={cartCount} hasActiveOrder={!!activeOrderId} onOpenCart={()=> setShowCart(true)} onOpenTracker={()=> setShowOrderTracker(true)} />
      <div className="fixed bottom-6 left-6 z-40"><button onClick={()=> setAudioNotifications(a=> !a)} className={`rounded-full h-10 w-10 flex items-center justify-center backdrop-blur border border-border/60 shadow-sm hover:shadow-md transition-all ${audioNotifications? 'bg-white/70 dark:bg-white/10':'bg-background/80'}`}>{audioNotifications? '🔊':'🔇'}</button></div>
