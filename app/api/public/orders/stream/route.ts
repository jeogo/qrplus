...
export async function GET(req: NextRequest) {
  ...
  interface PublicOrderDoc { id:number; table_id:number; status:string; updated_at?:string; last_status_at?:string }
  const stream = new ReadableStream<Uint8Array>({
    start(controller) {
      ...
      const unsub = orderRef.onSnapshot(snap => {
        if (!snap.exists) {
  ...
  const d = snap.data() as PublicOrderDoc
        send('status', { id: d.id, table_id: d.table_id, status: d.status, updated_at: d.updated_at, last_status_at: d.last_status_at })
      }, err => {
