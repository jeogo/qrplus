...
export async function POST(req: NextRequest, { params }: { params: { restaurantId: string; tableId: string } }) {
  try {
    ...
    prodSnap.docs.forEach(d => { const data = d.data(); if (data.account_id === restaurantIdNum && data.available === true) prodMap.set(data.id, data) })
    ...
    const orderDoc: { id:number; account_id:number; table_id:number; status:'pending'; total:number; created_at:string; updated_at:string; last_status_at:string; note?:string } = { id: orderId, account_id: restaurantIdNum, table_id: tableIdNum, status:'pending', total, created_at: now, updated_at: now, last_status_at: now }
    ...
    await mirrorOrderCreate(restaurantIdNum, hasNote ? { id: orderId, table_id: tableIdNum, status:'pending', total, created_at: now, updated_at: now, last_status_at: now, note } : { id: orderId, table_id: tableIdNum, status:'pending', total, created_at: now, updated_at: now, last_status_at: now })

    sendOrderNewPush({ id: orderId, table_id: tableIdNum, daily_number: undefined, account_id: restaurantIdNum }).catch(e=>console.error('[PUSH][PUBLIC_ORDER_NEW+RID]', e))
