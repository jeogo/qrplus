...
export async function GET(_req: Request, { params }: { params: { restaurantId: string; tableId: string } }) {
  try {
    ...
    const catSnap = await db.collection('categories').where('account_id','==',restaurantIdNum).where('active','==',true).orderBy('name').get()
    const categories = catSnap.docs.map(d=>d.data())
    if (!categories.length) return NextResponse.json({ success:true, data: [] })
    const prodSnap = await db.collection('products').where('account_id','==',restaurantIdNum).where('available','==',true).get()
    ...
    if (process.env.NODE_ENV !== 'production') console.error('[PUBLIC][MENU][CATEGORIES+RID]', err)
